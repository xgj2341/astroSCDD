...
def normalize_spectrum(wave, flux, norm_range, dwave,
    ...
    for i_bin in range(n_bin):
        ...
        if np.sum(ind_bin > 0):
            ...
            ind_good_ = ind_bin * (
                np.abs(dflux - np.nanpercentile(dflux[ind_bin], q * 100.)) < 1.5*(
                rsv_frac * bin_std))
    ...
    plt.plot(wave,flux_smoothed1,zorder=2)
    plt.plot(wave,flux_smoothed2,zorder=3)
    plt.scatter(wave[~ind_good],flux[~ind_good], c='r', marker='o', s=5, zorder=4)
    plt.show()
    s = 1
